...
export interface LiquidityZone {
  type: "buy_side" | "sell_side"; // Above highs (buy-side) or below lows (sell-side)
  level: number;
 ...
 */
export function findOrderBlocks(candles: OHLCV[], lookback: number = 50): OrderBlock[] {
  ...
  for (let i = 2; i < Math.min(candles.length - 1, lookback); i++) {
    ...
    if (
      ...
      if (displacement > 0.02) ob.strength = "strong";
      if (displacement < 0.01) ob.strength = "weak";
 ...
 */
export function findFairValueGaps(candles: OHLCV[], lookback: number = 50): FairValueGap[] {
...
export function findLiquidityZones(
  candles: OHLCV[],
  swingPoints: SwingPoint[]
): { buySide: LiquidityZone[]; sellSide: LiquidityZone[] } {
  const buySide: LiquidityZone[] = [];
  const sellSide: LiquidityZone[] = [];
  const currentPrice = candles[candles.length - 1].close;
  ...
  highs.forEach((high) => {
    const existing = buySide.find(
      (z) => Math.abs(z.level - high.price) / high.price < clusterThreshold
    );
    if (existing) {
      existing.strength++;
  ...
  // Cluster nearby lows
  lows.forEach((low) => {
    const existing = sellSide.find(
      (z) => Math.abs(z.level - low.price) / low.price < clusterThreshold
    );
    if (existing) {
      existing.strength++;
...
export function analyzeSMC(candles: OHLCV[]): SMCAnalysis {
  ...
  if (nearestOB) {
    if (nearestOB.type === "bullish" && currentPrice <= nearestOB.top * 1.01) {
      biasScore += 20;
      reasoning.push(
        `Price at bullish order block ($${nearestOB.bottom.toFixed(2)} - $${nearestOB.top.toFixed(2)})`
      );
    } else if (nearestOB.type === "bearish" && currentPrice >= nearestOB.bottom * 0.99) {
      biasScore -= 20;
      reasoning.push(
        `Price at bearish order block ($${nearestOB.bottom.toFixed(2)} - $${nearestOB.top.toFixed(2)})`
      );
    }
  }
  ...
  // Unfilled FVGs
  const bullishUnfilled = unfilledFVGs.filter(
    (f) => f.type === "bullish" && f.bottom > currentPrice
  );
  ...
  if (liquidity.buySide.length > 0) {
    ...
    reasoning.push(
      `Buy-side liquidity at $${nearestBuySide.level.toFixed(2)} (${nearestBuySide.strength} touches)`
    );
  }
  if (liquidity.sellSide.length > 0) {
    const nearestSellSide = liquidity.sellSide[0];
    reasoning.push(
      `Sell-side liquidity at $${nearestSellSide.level.toFixed(2)} (${nearestSellSide.strength} touches)`
    );
  }

  const biasDirection: "bullish" | "bearish" | "neutral" =
    biasScore > 20 ? "bullish" : biasScore < -20 ? "bearish" : "neutral";
