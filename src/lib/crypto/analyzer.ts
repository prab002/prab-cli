...
export type TiltDirection = "bullish" | "bearish" | "flat";
...
export interface EMATilt {
  direction: TiltDirection;
  angle: number; // Normalized angle (-100 to 100)
  strength: ProbabilityLevel;
  slopePercent: number; // Percentage change
}

export interface MarketConditionCheck {
  isSideways: boolean;
  emasIntersecting: boolean;
  emaSeparationPercent: number;
  emaAngleInOptimalRange: boolean;
  shouldTrade: boolean;
  reason: string;
}
...
export interface TechnicalIndicators {
  ...
  alertTrigger: boolean; // True when high probability setup detected
  marketCondition: MarketConditionCheck; // Sideways/intersection detection
}
 ...
 */
export function calculateEMATilt(emaValues: number[], lookback: number = 3): EMATilt {
...
export function checkMarketConditions(
  ...
  const tiltsConflicting =
    ema10Tilt.direction !== "flat" &&
 ...
 */
function isHighProbabilitySetup(
  currentPrice: number,
  ema10: number,
  ema10Tilt: EMATilt,
  ema20: number
...
function calculateTrendStrength(
  ...
  if (ema10 > ema20) strength += 15;
  if (ema20 > ema50) strength += 15;
  if (ema200 !== undefined && ema50 > ema200) strength += 15;

  // Check price vs EMAs
  if (currentPrice > ema10) strength += 10;
  if (currentPrice > ema20) strength += 10;
  if (currentPrice > ema50) strength += 10;
  if (ema200 !== undefined && currentPrice > ema200) strength += 10;

  // Tilt adds significant strength
  if (ema10Tilt.strength === "HIGH") strength += 15;
  else if (ema10Tilt.strength === "MEDIUM") strength += 10;
  else strength += 5;
...
function determineProbability(
  ...
  if (ema10Tilt.strength === "HIGH") score += 3;
  else if (ema10Tilt.strength === "MEDIUM") score += 2;
  else score += 1;

  if (ema20Tilt.strength === "HIGH") score += 2;
  else if (ema20Tilt.strength === "MEDIUM") score += 1;
  ...
  if (crossover !== "none") score += 2;

  // Clear price position = higher probability
  if (priceVsEMA !== "mixed") score += 1;
...
export function calculateIndicators(data: CryptoData): TechnicalIndicators {
  ...
  const alertTrigger = isHighProbabilitySetup(
    data.currentPrice,
    currentEMA10,
    ema10Tilt,
    currentEMA20
  ...
  const marketCondition = checkMarketConditions(
    ema.ema10,
    ema.ema20,
    ema10Tilt,
    ema20Tilt,
...
export function generateSignal(data: CryptoData): TradingSignal {
  ...
  if (!marketCondition.shouldTrade) {
    ...
    if (marketCondition.emasIntersecting) {
      reasoning.push(
        `EMA separation: ${marketCondition.emaSeparationPercent.toFixed(3)}% (too close)`
      );
    }
    ...
    if (!marketCondition.emaAngleInOptimalRange) {
      ...
      if (absSlope < 0.08) {
        reasoning.push(
          `EMA10 slope: ${ema10Tilt.slopePercent.toFixed(3)}% (too flat for reliable signal)`
        );
      } else if (absSlope > 1.2) {
        reasoning.push(
          `EMA10 slope: ${ema10Tilt.slopePercent.toFixed(3)}% (too steep, may reverse)`
        );
      }
    }
  ...
  reasoning.push(
    `✓ EMA separation: ${marketCondition.emaSeparationPercent.toFixed(2)}% - clear trend`
  );
  ...
  if (ema10Tilt.direction === "bullish") {
    if (ema10Tilt.strength === "HIGH") {
      ...
      confidence += 30;
      reasoning.push(
        `EMA10 strongly tilted UP (${ema10Tilt.slopePercent.toFixed(3)}%) - HIGH probability`
      );
    } else if (ema10Tilt.strength === "MEDIUM") {
      signal = "BUY";
      confidence += 20;
      reasoning.push(
        `EMA10 tilted UP (${ema10Tilt.slopePercent.toFixed(3)}%) - MEDIUM probability`
      );
    } else {
    ...
    if (ema10Tilt.strength === "HIGH") {
      ...
      confidence += 30;
      reasoning.push(
        `EMA10 strongly tilted DOWN (${ema10Tilt.slopePercent.toFixed(3)}%) - HIGH probability`
      );
    } else if (ema10Tilt.strength === "MEDIUM") {
      signal = "SELL";
      confidence += 20;
      reasoning.push(
        `EMA10 tilted DOWN (${ema10Tilt.slopePercent.toFixed(3)}%) - MEDIUM probability`
      );
    } else {
  ...
  const distanceFromEMA10 =
    ((data.currentPrice - indicators.currentEMA10) / indicators.currentEMA10) * 100;
  if (Math.abs(distanceFromEMA10) < 0.3) {
    ...
    confidence -= 5;
    reasoning.push(
      `Price extended from EMA10 (${distanceFromEMA10.toFixed(2)}%) - wait for pullback`
    );
  }
  ...
  if (signal === "BUY") {
    ...
    const distanceToEMA20 =
      ((data.currentPrice - indicators.currentEMA20) / data.currentPrice) * 100;
    stopLoss = Math.max(1.5, Math.min(distanceToEMA20 + 0.5, 4));
    takeProfit = stopLoss * 2.5; // 2.5:1 risk-reward
  } else if (signal === "SELL") {
    const distanceToEMA20 =
      ((indicators.currentEMA20 - data.currentPrice) / data.currentPrice) * 100;
