...
export interface OTE {
  isValid: boolean;
  fibLevel: number; // 0.62 - 0.79 is optimal
  zone: { low: number; high: number };
  swingHigh: number;
  swingLow: number;
  direction: "bullish" | "bearish";
  quality: "premium" | "standard" | "suboptimal";
}
...
export interface Inducement {
  ...
  trapType: "equal_highs" | "equal_lows" | "swing_high" | "swing_low";
}

export interface Displacement {
  detected: boolean;
  direction: "bullish" | "bearish";
  strength: number; // ATR multiplier
  startIndex: number;
  fvgCreated: boolean;
}

export interface PowerOf3 {
  phase: "accumulation" | "manipulation" | "distribution" | "unknown";
  asianRangeHigh: number;
  asianRangeLow: number;
  manipulation: "above" | "below" | "none";
  expectedMove: "bullish" | "bearish" | "unclear";
}
...
export interface ICTSignal {
  ...
  confidence: number;
  modelType: "silver_bullet" | "ote" | "breaker" | "displacement" | "standard";
...
const KILLZONES: Omit<Killzone, "active" | "bias">[] = [
  {
    name: "Asian Session",
    type: "asian",
...
function getCurrentKillzone(): Killzone {
  const now = new Date();
...
function calculateOTE(candles: OHLCV[], swingPoints: SwingPoint[], currentPrice: number): OTE {
  ...
  const direction: "bullish" | "bearish" = currentPrice > avgClose ? "bullish" : "bearish";

  // OTE Zone is 62% - 79% retracement
  let oteZone: { low: number; high: number };
  ...
  let quality: "premium" | "standard" | "suboptimal";
  if (direction === "bullish") {
    if (fibLevel >= 0.62 && fibLevel <= 0.79) quality = "premium";
    else if (fibLevel >= 0.5 && fibLevel <= 0.85) quality = "standard";
    else quality = "suboptimal";
  } else {
    if (fibLevel >= 0.21 && fibLevel <= 0.38) quality = "premium";
    else if (fibLevel >= 0.15 && fibLevel <= 0.5) quality = "standard";
    else quality = "suboptimal";
  }
...
function findBreakerBlocks(candles: OHLCV[], orderBlocks: OrderBlock[]): BreakerBlock[] {
...
function findInducements(candles: OHLCV[], swingPoints: SwingPoint[]): Inducement[] {
  ...
  for (let i = 1; i < highs.length; i++) {
    ...
    if (diff < 0.003) {
      ...
      // Check if swept
      let swept = false;
      let sweepCandle: number | null = null;
...
function detectDisplacement(candles: OHLCV[], atrValue: number): Displacement {
  ...
  for (let i = 0; i < recentCandles.length - 1; i++) {
    ...
    if (body > atrValue * 2) {
      ...
      if (i > 0 && i < recentCandles.length - 1) {
        ...
        if (direction === "bullish" && next.low > prev.high) fvgCreated = true;
        if (direction === "bearish" && next.high < prev.low) fvgCreated = true;
      }
...
function analyzePowerOf3(candles: OHLCV[]): PowerOf3 {
  ...
  let manipulation: "above" | "below" | "none" = "none";
  if (recentHigh > asianRangeHigh * 1.001) manipulation = "above";
  else if (recentLow < asianRangeLow * 0.999) manipulation = "below";

  // Determine phase
  let phase: "accumulation" | "manipulation" | "distribution" | "unknown";
  ...
  let expectedMove: "bullish" | "bearish" | "unclear" = "unclear";
...
export async function generateICTSignal(
  symbol: string,
  interval: TimeInterval = "1h"
): Promise<ICTSignal> {
  const spinner = ora(`Analyzing ${symbol} with ICT methodology...`).start();
  ...
  try {
    ...
    if (lastBOS) trend = lastBOS.direction;
    ...
    const orderBlocks = findOrderBlocks(candles, 100);
    const fvgs = findFairValueGaps(candles, 50);
    ...
    let confidence = 30;
    let modelType: ICTSignal["modelType"] = "standard";
    let entry = {
      price: currentPrice,
    ...
    const activeBreaker =
      breakerBlocks.find((bb) => {
        return currentPrice >= bb.bottom * 0.995 && currentPrice <= bb.top * 1.005;
      }) || null;

    // Recent sweep detection
    const recentSweep =
      inducements.find((ind) => {
        if (!ind.swept || ind.sweepCandle === null) return false;
    ...
    if (killzone.type === "silver_bullet_am" || killzone.type === "silver_bullet_pm") {
      ...
      const trendFVG = unfilledFVGs.find(
        (f) => f.type === (trend === "bullish" ? "bullish" : "bearish")
      );
    ...
    if (signal === "WAIT" && displacement.detected && displacement.fvgCreated) {
      ...
      const displacementFVG = fvgs.find(
        (f) =>
          f.type === displacement.direction && !f.filled && f.index >= displacement.startIndex - 2
      );
    ...
    // ============================================
    if (killzone.active) confluenceFactors.push(`Active Killzone: ${killzone.name}`);
    if (mss) confluenceFactors.push("Recent Market Structure Shift");
    if (recentSweep) confluenceFactors.push(`${recentSweep.type} liquidity swept`);
    ...
    reasoning.push(
      `Premium/Discount: ${premiumDiscount.zone} (${premiumDiscount.fibLevel.toFixed(0)}%)`
    );
...
export function displayICTSignal(result: ICTSignal): void {
  console.log("");
  ...
  console.log(
    headerColor(
      `  ║       ${signalIcon} ICT TRADING STRATEGY - ${result.symbol.padEnd(20)}          ║`
    )
  );
  console.log(
    headerColor("  ╚═══════════════════════════════════════════════════════════════════════╝")
  ...
  const modelBadge = chalk.bgBlue.white(` ${result.modelType.toUpperCase().replace("_", " ")} `);

  console.log(
    `  ${signalBadge} ${modelBadge}  Confidence: ${getConfidenceBar(result.confidence)} ${result.confidence}%`
  );
  console.log("");
  ...
  console.log(
    chalk.magenta("  │                      ⏰ KILLZONE STATUS                             │")
  );
  console.log(
    chalk.magenta("  ├─────────────────────────────────────────────────────────────────────┤")
  );

  const kzColor = result.inKillzone ? chalk.green : chalk.yellow;
  console.log(
    `  │  ${kzColor(result.killzone.name)} ${result.inKillzone ? chalk.green("● ACTIVE") : chalk.yellow("○ INACTIVE")}`
  );
  console.log(`  │  ${chalk.dim(result.killzone.description)}`);

  // Show all killzones with times
  console.log(`  │`);
  console.log(`  │  ${chalk.dim("Session Times (UTC):")}`);
  console.log(`  │  ${chalk.cyan("Asian:")} 00:00-06:00  ${chalk.yellow("London:")} 07:00-10:00`);
  console.log(`  │  ${chalk.green("NY AM:")} 13:00-16:00  ${chalk.red("NY PM:")} 18:00-21:00`);
  console.log(`  │  ${chalk.magenta("Silver Bullet:")} 14:00-15:00 & 19:00-20:00`);
  ...
  console.log(
    chalk.cyan("  │                      📐 OTE (OPTIMAL TRADE ENTRY)                   │")
  );
  console.log(
    chalk.cyan("  ├─────────────────────────────────────────────────────────────────────┤")
  );

  const oteQualityColor =
    result.ote.quality === "premium"
      ? chalk.green
      : result.ote.quality === "standard"
        ? chalk.yellow
        : chalk.red;

  console.log(`  │  ${chalk.dim("Quality:")} ${oteQualityColor(result.ote.quality.toUpperCase())}`);
  console.log(
    `  │  ${chalk.dim("Fib Level:")} ${(result.ote.fibLevel * 100).toFixed(1)}% (Optimal: 62-79%)`
  );
  console.log(
    `  │  ${chalk.dim("OTE Zone:")} ${formatPrice(result.ote.zone.low)} - ${formatPrice(result.ote.zone.high)}`
  );
  console.log(
    `  │  ${chalk.dim("Swing Range:")} ${formatPrice(result.ote.swingLow)} - ${formatPrice(result.ote.swingHigh)}`
  );
  ...
  console.log(
    chalk.yellow("  │                      ⚡ POWER OF 3 (AMD MODEL)                      │")
  );
  console.log(
    chalk.yellow("  ├─────────────────────────────────────────────────────────────────────┤")
  );
  ...
  const phaseColor =
    result.powerOf3.phase === "accumulation"
      ? chalk.blue
  ...
  console.log(
    `  │  ${chalk.dim("Current Phase:")} ${phaseColor(result.powerOf3.phase.toUpperCase())}`
  );
  if (result.powerOf3.asianRangeHigh > 0) {
    console.log(
      `  │  ${chalk.dim("Asian Range:")} ${formatPrice(result.powerOf3.asianRangeLow)} - ${formatPrice(result.powerOf3.asianRangeHigh)}`
    );
  }
  const manipColor =
    result.powerOf3.manipulation === "above"
      ? chalk.red
      : result.powerOf3.manipulation === "below"
        ? chalk.green
        : chalk.gray;
  console.log(`  │  ${chalk.dim("Manipulation:")} ${manipColor(result.powerOf3.manipulation)}`);
  const expectedColor =
    result.powerOf3.expectedMove === "bullish"
      ? chalk.green
      : result.powerOf3.expectedMove === "bearish"
        ? chalk.red
        : chalk.yellow;
  console.log(`  │  ${chalk.dim("Expected Move:")} ${expectedColor(result.powerOf3.expectedMove)}`);
  ...
  const trendColor =
    result.marketStructure.trend === "bullish"
      ? chalk.green
      : result.marketStructure.trend === "bearish"
        ? chalk.red
        : chalk.yellow;
  console.log(
    `  │  ${chalk.dim("Trend:")} ${trendColor(result.marketStructure.trend.toUpperCase())}`
  );

  if (result.marketStructure.lastBOS) {
    const bosColor =
      result.marketStructure.lastBOS.direction === "bullish" ? chalk.green : chalk.red;
    console.log(
      `  │  ${chalk.dim("Last BOS:")} ${bosColor(result.marketStructure.lastBOS.direction)} @ ${formatPrice(result.marketStructure.lastBOS.level)}`
    );
  }
  if (result.marketStructure.lastCHoCH) {
    const chochColor =
      result.marketStructure.lastCHoCH.direction === "bullish" ? chalk.green : chalk.red;
    console.log(
      `  │  ${chalk.dim("Last CHoCH:")} ${chochColor(result.marketStructure.lastCHoCH.direction)} @ ${formatPrice(result.marketStructure.lastCHoCH.level)}`
    );
  }
  console.log(
    `  │  ${chalk.dim("MSS (Shift):")} ${result.marketStructure.mss ? chalk.green("YES") : chalk.gray("NO")}`
  );

  const pdColor =
    result.premiumDiscount.zone === "discount"
      ? chalk.green
      : result.premiumDiscount.zone === "premium"
        ? chalk.red
        : chalk.yellow;
  console.log(
    `  │  ${chalk.dim("Zone:")} ${pdColor(result.premiumDiscount.zone)} (${result.premiumDiscount.fibLevel.toFixed(0)}%)`
  );
  ...
  console.log(
    chalk.gray("  │                      💥 DISPLACEMENT & INDUCEMENT                   │")
  );
  console.log(
    chalk.gray("  ├─────────────────────────────────────────────────────────────────────┤")
  );
  ...
  if (result.displacement.detected) {
    const dispColor = result.displacement.direction === "bullish" ? chalk.green : chalk.red;
    console.log(
      `  │  ${chalk.dim("Displacement:")} ${dispColor(result.displacement.direction)} (${result.displacement.strength.toFixed(1)}x ATR)`
    );
    console.log(
      `  │  ${chalk.dim("FVG Created:")} ${result.displacement.fvgCreated ? chalk.green("YES") : chalk.gray("NO")}`
    );
  } else {
    console.log(`  │  ${chalk.dim("Displacement:")} ${chalk.gray("None detected")}`);
  }

  console.log(`  │`);
  console.log(`  │  ${chalk.dim("Inducements:")} ${result.inducements.length} found`);
  if (result.recentSweep) {
    const sweepColor = result.recentSweep.type === "buy_side" ? chalk.red : chalk.green;
    console.log(
      `  │  ${chalk.yellow("⚠ Recent Sweep:")} ${sweepColor(result.recentSweep.type)} @ ${formatPrice(result.recentSweep.level)}`
    );
  }
  ...
  if (result.breakerBlocks.length > 0) {
    ...
    result.breakerBlocks.slice(0, 4).forEach((bb) => {
      const bbColor = bb.type === "bullish" ? chalk.green : chalk.red;
      const isActive = result.activeBreaker === bb;
      const marker = isActive ? chalk.yellow(" ← ACTIVE") : "";
      console.log(
        `  │  ${bbColor(bb.type.padEnd(8))} ${formatPrice(bb.bottom)} - ${formatPrice(bb.top)} [${bb.strength}]${marker}`
      );
    });
  ...
  if (result.signal !== "WAIT") {
    ...
    console.log(`  │  ${chalk.dim(result.entry.reason)}`);
    console.log(`  │`);
    console.log(
    ...
    console.log(`  │`);
    console.log(
      `  │  ${chalk.green("TP1:")} ${formatPrice(result.takeProfit1)} ${chalk.dim("(2R)")}`
    );
    console.log(
      `  │  ${chalk.green("TP2:")} ${formatPrice(result.takeProfit2)} ${chalk.dim("(3R)")}`
    );
    console.log(
      `  │  ${chalk.green("TP3:")} ${formatPrice(result.takeProfit3)} ${chalk.dim("(5R)")}`
    );
    console.log(`  │`);
    console.log(
      `  │  ${chalk.cyan("R:R Ratio:")} ${chalk.cyan.bold(result.riskRewardRatio.toFixed(1) + ":1")}`
    );
  ...
  console.log(
    chalk.dim("  │                      📚 ICT CONCEPTS GUIDE                          │")
  );
  console.log(
    chalk.dim("  ├─────────────────────────────────────────────────────────────────────┤")
  );
  console.log(
    chalk.dim("  │  OTE: Optimal Trade Entry (62-79% Fib retracement)                 │")
  );
  console.log(
    chalk.dim("  │  BOS: Break of Structure | CHoCH: Change of Character              │")
  );
  console.log(
    chalk.dim("  │  MSS: Market Structure Shift (trend reversal signal)               │")
  );
  console.log(
    chalk.dim("  │  AMD: Accumulation → Manipulation → Distribution                   │")
  );
  console.log(
    chalk.dim("  │  Silver Bullet: High-probability 1hr entry windows                 │")
  );
  console.log(
    chalk.dim("  │  Breaker: Failed OB that flips to support/resistance               │")
  );
  console.log(
    chalk.dim("  └─────────────────────────────────────────────────────────────────────┘")
  );
...
export async function runICTStrategy(
  symbol: string,
  interval: TimeInterval = "1h"
