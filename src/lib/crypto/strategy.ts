...
import { OHLCV, CryptoData, fetchCryptoData, TimeInterval } from "./data-fetcher";
import { calculateEMA, calculateEMATilt, EMATilt } from "./analyzer";

// ============================================
// TYPES AND INTERFACES
// ============================================

export type StrategySignal = "STRONG_BUY" | "BUY" | "WAIT" | "SELL" | "STRONG_SELL" | "NO_TRADE";
...
export interface KeyLevel {
  price: number;
  type: "support" | "resistance" | "order_block_bullish" | "order_block_bearish";
  strength: number; // 0-100 based on touches and recency
  distance: number; // percentage from current price
}

export interface PullbackStatus {
  isPullback: boolean;
  inEMAZone: boolean; // Price between EMA10 and EMA20
  distanceToEMA10: number;
  distanceToEMA20: number;
  quality: "optimal" | "acceptable" | "extended" | "none";
}
...
export interface ConfluenceScore {
  total: number; // 0-100
  breakdown: {
    trendAlignment: number; // Max 30 points
    pullbackQuality: number; // Max 25 points
    keyLevel: number; // Max 20 points
    volume: number; // Max 15 points
    rsiRange: number; // Max 10 points
  };
...
export interface StrategyResult {
  ...
  higherTimeframe: TimeframeAnalysis;
  lowerTimeframe: TimeframeAnalysis;
 ...
 */
function getCandleMetrics(candle: OHLCV) {
 ...
 */
export function detectCandlePattern(candles: OHLCV[]): CandlePattern {
  if (candles.length < 3) {
  ...
  if (
    currentMetrics.lowerWickRatio > 0.6 &&
  ...
  if (
    currentMetrics.upperWickRatio > 0.6 &&
    currentMetrics.bodyRatio < 0.3 &&
  ...
  if (
    currentMetrics.lowerWickRatio > 0.5 &&
 ...
 */
export function detectKeyLevels(candles: OHLCV[], currentPrice: number): KeyLevel[] {
  ...
  const groupLevels = (prices: number[], type: "support" | "resistance"): KeyLevel[] => {
  ...
  return [...supports, ...resistances, ...orderBlocks].sort(
 ...
 */
function detectOrderBlocks(candles: OHLCV[], currentPrice: number): KeyLevel[] {
  const orderBlocks: KeyLevel[] = [];
  ...
  for (let i = 5; i < lookback; i++) {
    ...
    const avgBodySize =
      candles.slice(idx - 10, idx).reduce((sum, c) => sum + Math.abs(c.close - c.open), 0) / 10;
...
export function detectPullback(
  ...
  const inEMAZone =
    (currentPrice >= Math.min(ema10, ema20) && currentPrice <= Math.max(ema10, ema20)) ||
    Math.abs(distanceToEMA10) < 0.3 ||
    Math.abs(distanceToEMA20) < 0.3;

  // Determine if this is a valid pullback
  let isPullback = false;
  let quality: "optimal" | "acceptable" | "extended" | "none" = "none";
 ...
 */
export function analyzeVolume(candles: OHLCV[]): VolumeAnalysis {
 ...
 */
export function analyzeTimeframe(data: CryptoData, interval: TimeInterval): TimeframeAnalysis {
  ...
  const emaTilt = calculateEMATilt(ema10);
...
export function calculateConfluenceScore(
  htfAnalysis: TimeframeAnalysis,
  ltfAnalysis: TimeframeAnalysis,
  pullback: PullbackStatus,
  keyLevels: KeyLevel[],
  volume: VolumeAnalysis,
  candlePattern: CandlePattern,
  ...
  const htfTrendMatch =
    (direction === "long" && htfAnalysis.trend === "bullish") ||
    (direction === "short" && htfAnalysis.trend === "bearish");

  const ltfTrendMatch =
    (direction === "long" && ltfAnalysis.trend === "bullish") ||
  ...
  if (nearbyLevel) {
    ...
    if (nearbyLevel.type.includes("order_block")) {
      factors.push(
        `✓ At ${nearbyLevel.type.replace(/_/g, " ")} ($${nearbyLevel.price.toFixed(2)})`
      );
    } else {
      factors.push(
        `✓ Near ${nearbyLevel.type} level ($${nearbyLevel.price.toFixed(2)}, ${Math.abs(nearbyLevel.distance).toFixed(1)}% away)`
      );
    }
  } else {
...
export async function generateStrategySignal(
  symbol: string,
  htfInterval: TimeInterval = "4h",
  ltfInterval: TimeInterval = "1h"
): Promise<StrategyResult> {
  // Fetch data for both timeframes
  const [htfData, ltfData] = await Promise.all([
    fetchCryptoData(symbol, htfInterval, 250),
    fetchCryptoData(symbol, ltfInterval, 250),
  ]);
  ...
  let signal: StrategySignal = "NO_TRADE";
  ...
  if (htfAnalysis.trend !== ltfAnalysis.trend && ltfAnalysis.trend !== "neutral") {
    warnings.push(
      `Timeframe conflict: ${htfInterval} is ${htfAnalysis.trend}, ${ltfInterval} is ${ltfAnalysis.trend}`
    );
  }
