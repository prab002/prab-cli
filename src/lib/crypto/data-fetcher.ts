...
const BYBIT_INTERVAL_MAP: Record<TimeInterval, string> = {
  "1m": "1",
  "5m": "5",
  "15m": "15",
  "1h": "60",
...
const CRYPTOCOMPARE_INTERVAL_MAP: Record<TimeInterval, { endpoint: string; aggregate: number }> = {
 ...
 */
async function fetchAllSymbolsFromBybit(): Promise<Set<string>> {
  try {
 ...
 */
function getCommonSymbols(): Set<string> {
  const common = [
    "BTC",
    "ETH",
    "BNB",
 ...
 */
async function fetchOHLCVFromBybit(
  symbol: string,
 ...
 */
async function fetch24hTickerFromBybit(symbol: string): Promise<{
  price: number;
  priceChange: number;
 ...
 */
function isGeoRestricted(error: string): boolean {
  return (
    error.includes("restricted location") ||
 ...
 */
function getBaseSymbol(normalizedSymbol: string): string {
 ...
 */
async function fetch24hTickerFromCryptoCompare(symbol: string): Promise<{
  price: number;
  priceChange: number;
  priceChangePercent: number;
  high24h: number;
...
export async function fetchOHLCV(
  ...
  throw new Error(
    `Failed to fetch data for ${normalizedSymbol}. Tried all providers: ${errors.join("; ")}`
  );
}
...
export async function fetch24hTicker(symbol: string): Promise<{
  ...
  throw new Error(
    `Failed to fetch ticker for ${normalizedSymbol}. Tried all providers: ${errors.join("; ")}`
  );
}
